...
extern void load_ui_multi(void* phy_fb, int color_bytes);
extern void load_ui_single(void* phy_fb, int color_bytes);
extern void load_mini_ui_single(void* phy_fb, int color_bytes);
extern void load_mini_ui_multi(void* phy_fb, int color_bytes);
...
int run(void** main_fbs, int main_cnt, void** sub_fbs, int sub_cnt, int color_bytes)
{
	...
	for (int i = 0; i < main_cnt; i++)
	{
		...
		{
			load_ui_single(main_fbs[i], color_bytes);
		}
		else
		{
			load_ui_multi(main_fbs[i], color_bytes);
		}
	}
	for (int i = 0; i < sub_cnt; i++)
	{
		...
		{
			load_mini_ui_single(sub_fbs[i], color_bytes);
		}
		else
		{
			load_mini_ui_multi(sub_fbs[i], color_bytes);
		}
	}
...
int run(int main_cnt, int sub_cnt, int color_bytes)
{
	void** main_fbs = (void**)malloc(sizeof(void*) * main_cnt);
	void** sub_fbs = (void**)malloc(sizeof(void*) * sub_cnt);
	for (int i = 0; i < main_cnt; i++)
	{
		main_fbs[i] = calloc(1024 * 768 * color_bytes, 1);
	}
	for (int i = 0; i < sub_cnt; i++)
	{
		sub_fbs[i] = calloc(1024 * 370 * color_bytes, 1);
	}
	return run(main_fbs, main_cnt, sub_fbs, sub_cnt, color_bytes);
}
